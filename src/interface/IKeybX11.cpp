...
$Id: IKeybX11.cpp,v 1.2 2002/11/29 14:26:06 hmichelon Exp $
--------------------------------------------------------------------------*/
...
{ 
	...
	if (keycode)
	{
		char keys[32];
		_WORD pos = keycode >> 3;
		_BYTE nbit = keycode - (pos << 3);
		_BYTE mask = 1;
		for (_BYTE i=0; i<nbit; i++) mask = mask << 1;
		XQueryKeymap(display, keys);
		return ((keys[pos] & mask)  != 0);
	}*/
	return FALSE;
}
...
//-------------------------------------------------------
void IKeyb :: SetEvent(_PTR EVT)
{
	XKeyEvent *evt = (XKeyEvent*)EVT;
	shift = (evt->state & ShiftMask) != 0;
	capslock = (evt->state & LockMask) != 0;
	ctrl = (evt->state & ControlMask) != 0;
	alt = (evt->state & Mod1Mask) != 0;
	os = (evt->state & Mod4Mask) != 0; // Left "Windows" Key under XFree86/x86
}
