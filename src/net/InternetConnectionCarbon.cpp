...
$Id: InternetConnectionCarbon.cpp,v 1.2 2002/11/29 14:26:06 hmichelon Exp $
-----------------------------------------------------------------------------*/
#ifdef macintosh
#include <nmo/NMO.hpp>
#include <nmo/Net.hpp>
...
{
	OSErr		err			= noErr;
	ICInstance	gICInstance	= NULL;
	long 		startSel;
	long 		endSel;
	
	err = ICStart(&gICInstance, '????');
	if (err != noErr) {
		dprintf("OpenURL: ICStart\n");
		return FALSE;
	}
	startSel = 0;
	endSel = URL.Len();	
	err = ICLaunchURL(gICInstance, "\p", (_CHAR*)URL, URL.Len(), &startSel, &endSel);
	Debug(if (err != noErr) { dprintf("OpenURL: ICLaunchURL\n"); });
	ICStop(gICInstance);
	return err == noErr;
}
