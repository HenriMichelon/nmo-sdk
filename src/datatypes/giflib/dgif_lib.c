...
int DGifSlurp(GifFileType *GifFile)
{
	...
	switch (RecordType) {
		...
		sp->RasterBits
		    = (char*)(GifPixelType*) malloc(ImageSize * sizeof(GifPixelType));
		...
		if (DGifGetExtension(GifFile,&sp2.Function,&ExtData)==GIF_ERROR)
		    return(GIF_ERROR);
		else
		{
		    /*//ep = &sp2->ExtensionBlocks[sp2->ExtensionBlockCount++];

		    //ep.ByteCount = ExtData[0];
		    //ep.Bytes = (char*)(GifByteType *)malloc(ep.ByteCount * sizeof(GifByteType));
